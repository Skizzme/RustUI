...
impl ScreenTrait for DefaultScreen {
    ...
    unsafe fn register_elements(&mut self) -> Vec<Element> {
        vec![
            Element::new(Bounds::xywh(5.0, 100.0, 100.0, 100.0), |el, event| {
                match event {
                    Event::Render(_) => {
                        ...
                        context().renderer().draw_rect_outline(*el.bounds(), 1.0, 0xffffff);
                    }
                    _ => {}
                }
            } ),
            Element::new(Bounds::xywh(200.0, 400.0, 100.0, 100.0), |el, event| {
                match event {
                    Event::Render(_) => {
                        let mouse = context().window().mouse();
